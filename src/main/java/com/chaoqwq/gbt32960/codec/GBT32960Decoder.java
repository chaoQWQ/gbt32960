...
import static com.chaoqwq.gbt32960.paltform.FrameHeader.HEADER_LENGTH;
...
public class GBT32960Decoder extends ReplayingDecoder<Void> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        checkpoint();
        log.info("收到GBT32960报文消息: {}", ByteBufUtil.hexDump(internalBuffer()));
        ...
        try {
            ...
            log.error("GBT32960报文头部解析出错:{}",e.getMessage(), e);
            in.skipBytes(actualReadableBytes());
            ctx.close();
            return;
        }
        //todo 处理数据加密过的情况（枫叶数据不加密，故暂不处理）
        //获取数据单元长度
...
/*        计算收到的数据的校验码 现在的in里虽然头部已经消费掉了##和header但是记录了原始的进来的in的索引位置，所以可以进行
        ...
        if (ResponseTag.COMMAND == frameHeader.getResponseTag() && payloadLength > 0) {
            // 进行信息解码，此命令结束in继续前移到倒数第一位
            dataUnit = decodeDataUnit(in, frameHeader);
        }
    ...
    private FrameHeader decodeFrameHeader(ByteBuf in) {
        frameHeader = FrameHeader.builder()
                ...
                // 读取vin唯一标志码
                .vin(in.readCharSequence(GBT32960Constant.VIN_LENGTH, GBT32960Message.ASCII_CHARSET).toString())
