...
public class GBT32960Message {
    ...
    public static void encodeRealTimeMessage(ChannelHandlerContext ctx, ByteBuf out, String vin, ResponseTag responseTag, Consumer<ByteBuf> payloadEncoder) {
        //写启始符
        out.writeShort(START_SYMBOL);
        //记录写完启始符后的out此时的指针位置
        int startIndex = out.writerIndex();
        //写命令标识
        out.writeByte(RequestType.REAL_TIME.getValue());
        //写应答标志
        out.writeByte(responseTag.getValue());
        //写vin（多个字节）
        out.writeBytes(vin.getBytes(ASCII_CHARSET));
...
//        out.writeByte(0);
//        out.writeInt(127123123);
//        out.writeInt(96123123);
        //数据单元开始写,消费之模式里的accept方法消费数据单元并写入out中
        payloadEncoder.accept(out);
        //计算数据单元长度
        int payloadLength = out.writerIndex() - payloadBegin;
